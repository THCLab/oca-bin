...
pub fn handle_get(said: SelfAddressingIdentifier, local_repository_path: PathBuf) -> Result<(), PresentationError> {
	let facade = get_oca_facade(local_repository_path);
	let oca_bundles = facade.get_oca_bundle(said, true).map_err(PresentationError::OcaBundleErrors)?;
