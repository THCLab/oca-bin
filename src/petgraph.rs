...
use crate::dependency_graph::{find_refn, Node};
...
impl PetGraph {
    ...
    pub fn neighbors(&self, refn: &str) -> Vec<Node> {
        let fourth = self.get_index(refn);
        self
            .graph
            .neighbors(fourth.unwrap())
...
fn test_sort() -> anyhow::Result<()> {
    ...
    let petgraph = PetGraph::new(paths);
    assert_eq!(petgraph.sort(), vec!["first", "second", "third", "fourth"]);

    let n: Vec<_> = petgraph.neighbors("fourth").iter().map(|n| n.refn.clone()).collect();
    assert_eq!(n.len(), 3);
